...
// Adresses for the three DAC modules
#define DAC2 0x60 
#define DAC3 0x61
#define DAC4 0x62
byte buffer [3];
...
int valveEjectedVolume = 1;// Ejected volume in microliters
unsigned long buttonPushedMillis; // when button was released
unsigned long valveTurnedOnAt; // when valve was turned on
unsigned long turnOnDelay = 0; // wait to turn on valve
unsigned long turnOffDelay = (valveEjectedVolume + 2.0778)/0.0847; // turn off valve after this time (Function extracted from the reward delivery system calibration equation) 
...
float conversion_factor = 123.59; //120 mouse units/1cm
...
float d_angle = 0;
float encoder_angle_0 = 0;
float max_angle_change = 2;
...
int licksensor = 0;
const int licksensor_out_pin = 2;
...
void setup() 
{
  ...
  //Licksensor out setup
  pinMode(licksensor_out_pin, OUTPUT);
  digitalWrite(licksensor_out_pin, LOW);
}
...
/////////ENCODER ANGLE/////////////////////////////////
  ...
  encoder_angle = (flag_A-flag_B)*deg_pulse;  // WE HAVE TO CHECK IF THE SENSE IS CORRECT!!!!

//Encoder delta angle

// If the new value of angle changes measure the dangle and update the initial angle:  
  if (encoder_angle_0 != encoder_angle)

  { 
      d_angle = encoder_angle-encoder_angle_0;
      encoder_angle_0 = encoder_angle;
  }
// Else the delta angle is zero
  else
  {
      d_angle = 0.00;
  }
...
////// ARDUINO DAC OUTPUTS/////////////////////////////////
  ...
  DAC_out_1 = int((2047.5/max_angle_change)*d_angle + 2047.5);
  
  analogWrite(DAC0, DAC_out_0);
  analogWrite(DAC1, DAC_out_1);

////// Licking sensor DAC output /////////////////////
  
  licksensor = analogRead(A0);
  DAC_out_4 = licksensor*4;
...
//  SerialUSB.print(licksensor);
//  SerialUSB.print(" ");
 Serial.print("Encoder d_angle: "); 
 Serial.println(d_angle);
//  SerialUSB.print("Encoder d_angle: "); 
//  SerialUSB.println(DAC_out_1);

// Pulse to measure the time execution
 digitalWrite(licksensor_out_pin, HIGH);   
 delay(1);                     
 digitalWrite(licksensor_out_pin, LOW);  
